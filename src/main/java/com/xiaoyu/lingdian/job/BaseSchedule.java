...
public abstract class BaseSchedule extends AbstractOneOffElasticJob {
    ...
    private Boolean getRunning() {
        String isRunning = this.redisCacheService.getString(this.getIsRunningType());
        return isRunning == null ? Boolean.valueOf(false) : Boolean.valueOf(isRunning.toString());
    }

    private void updateRunState() {
        this.redisCacheService.set(this.getIsRunningType(), "true", 600l);
    }

    private void completeRunning() {
        this.redisCacheService.set(this.getIsRunningType(), "false", 600l);
    }

    private void updateLastRunTime(Date lastRunTime) {
        this.redisCacheService.set(this.getLastTimeType(), lastRunTime.toString(), 2147483647l);
    }

    protected Date getLastRunTime() {
    	String lastRunTime = this.redisCacheService.getString(this.getLastTimeType());
